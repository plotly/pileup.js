...
CoverageTiledCanvas = (function (_TiledCanvas) {_inherits(CoverageTiledCanvas, _TiledCanvas);
  ...
  _createClass(CoverageTiledCanvas, [{ key: 'heightForRef', value: function heightForRef(ref) {return this.height;} }, { key: 'update', value: function update(height, options) {// workaround for an issue in PhantomJS where height always comes out to zero.
      this.height = Math.max(1, height);this.options = options;} }, { key: 'yScaleForRef', value: function yScaleForRef(ref, bottomPadding, topPadding) {var maxCoverage = this.cache.maxCoverageForRef(ref);return _scale2['default'].linear().domain([maxCoverage, 0]).range([bottomPadding, this.height - topPadding]).nice();} }, { key: 'render', value: function render(ctx, xScale, range) {var bins = this.cache.binsForRef(range.contig);var yScale = this.yScaleForRef(range.contig, 10, 10);var relaxedRange = new _ContigInterval2['default'](range.contig, range.start() - 1, range.stop() + 1);renderBars(ctx, xScale, yScale, relaxedRange, bins, this.options);} }]);return CoverageTiledCanvas;})(_TiledCanvas3['default']);function renderBars(ctx, xScale, 
yScale, 
range, 
bins, 
options) {
  ...
  for (var pos = start; pos < stop; pos++) {
    ...
    if (!lastPos) {var _binPos = 
      binPos(pos, bin.count);var _barX1 = _binPos.barX1;
      ctx.fillStyle = _style2['default'].COVERAGE_BIN_COLOR;
      ctx.beginPath();
      ctx.moveTo(_barX1, vBasePosY);}


    ctx.pushObject(bin);var _binPos2 = 
    binPos(pos, bin.count);var barX1 = _binPos2.barX1;var barX2 = _binPos2.barX2;var barY = _binPos2.barY;
  ...
  if (lastPos) {var _binPos3 = 
    binPos(lastPos, 0);var barX2 = _binPos3.barX2; // count does not matter, drawing vBasePosY
  ...
  function CoverageTrack(props) {_classCallCheck(this, CoverageTrack);
    ...
    function componentDidMount() {var _this = this;
      ...
      this.props.source.on('newdata', function (range) {
        ...
        _this.props.source.getFeaturesInRange(range).
        forEach(function (read) {return _this.cache.addItem(read);});
...
module.exports = CoverageTrack; // no state, used to make flow happy