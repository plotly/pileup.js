...
vGroups) {
  ...
  function drawArrow(pos, refLength, top, direction) {
    ...
    bottom = top + _style2['default'].READ_HEIGHT, 
    ...
    switch (op.op) {
      case _pileuputils.CigarOp.MATCH:
          ...
          drawArrow(op.pos, op.length, y, op.arrow);} else 
        {
          ...
          ctx.fillRect(x, y, scale(op.pos + op.length + 1) - x, _style2['default'].READ_HEIGHT);}
      ...
      case _pileuputils.CigarOp.DELETE:
        ...
        yp = y + _style2['default'].READ_HEIGHT / 2 - 0.5;
        ctx.save();
      ...
      case _pileuputils.CigarOp.INSERT:
        ...
        y1 = y - 1, 
        y2 = y + _style2['default'].READ_HEIGHT + 2;
  ...
  function drawGroup(vGroup) {
    ...
    var y = (0, _pileuputils.yForRow)(vGroup.row);
    ctx.pushObject(vGroup);
    if (vGroup.insert) {
      var span = vGroup.insert, 
      x1 = scale(span.start + 1), 
      x2 = scale(span.stop + 1);
      ctx.fillRect(x1, y + _style2['default'].READ_HEIGHT / 2 - 0.5, x2 - x1, 1);}

    vGroup.items.forEach(function (vRead) {return drawAlignment(vRead, y);});
    ctx.popObject();
    ctx.restore();}
  ...
  function renderMismatch(bp, y) {
    ...
    if (showText) {
      // 0.5 = centered
      ctx.fillText(bp.basePair, scale(1 + 0.5 + bp.pos), y + _style2['default'].READ_HEIGHT - 2);} else 
    {
      ctx.fillRect(scale(1 + bp.pos), y, pxPerLetter - 1, _style2['default'].READ_HEIGHT);}

    ctx.restore();
    ctx.popObject();}


  ctx.font = _style2['default'].TEXT_STYLE(_DisplayMode2['default'].TIGHT, _style2['default'].READ_HEIGHT - 1);
  ...
  function PileupTrack(props) {_classCallCheck(this, PileupTrack);
    ...
    function render() {
      ...
      if (networkStatus) {
        ...
        statusEl = 
        _ShimDependencies.React.createElement('div', { ref: 'status', className: 'network-status' }, 
        _ShimDependencies.React.createElement('div', { className: 'network-status-message' }, 'Loading alignments… (', 
        message, ')'));}
      ...
      return (
        ...
        _ShimDependencies.React.createElement('canvas', { ref: 'canvas', onClick: this.handleClick.bind(this) }))));} }, { key: 'formatStatus', value: 
    ...
    function handleOptionsChange(oldOpts) {
      ...
      if (oldOpts.colorByInsert != this.props.options.colorByInsert) {
        ...
        this.tiles.invalidateAll();
        this.updateVisualization();} else 
      if (oldOpts.hideAlignments != this.props.options.hideAlignments) {
        this.tiles.update(this.props.options);
    ...
    function renderScene(ctx) {
      ...
      var parent = _d3utils2['default'].findParent(canvas, "pileup");

      if (this.props.options.hideAlignments) {
        _d3utils2['default'].sizeCanvas(canvas, width, 60);
    ...
    function handleClick(reactEvent) {
      ...
      if (vRead) {
        alert(vRead.read.debugString());}} }]);return PileupTrack;})(_ShimDependencies.React.Component);
