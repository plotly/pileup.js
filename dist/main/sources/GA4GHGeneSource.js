...
'../ContigInterval');var _ContigInterval2 = _interopRequireDefault(_ContigInterval);var _dataGene = require(



'../data/gene');var _dataGene2 = _interopRequireDefault(_dataGene);
...
function create(spec) {
  ...
  function addFeaturesFromResponse(response) {
    ...
    response.features.forEach(function (ga4ghFeature) {
      ...
      var gene = _dataGene2['default'].fromGA4GH(ga4ghFeature);
      genes[key] = gene;});}
  ...
  function getFeaturesInRange(range) {
    if (!range) return [];
    return _underscore2['default'].filter(genes, function (gene) {return gene.intersects(range);});}
